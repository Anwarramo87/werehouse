...
- الحقول الأساسية في التنفيذ الحالي: `id`, `name`, `createdAt`, `updatedAt`.
- لا يوجد حقل `manager` داخل الموديل الحالي، ويمكن إضافته لاحقاً إذا احتاجه البزنس.
...
### 10.4 دورة حياة الموظف (الإقالة والتصفية)

- تم إضافة حقول جديدة للموظف: `terminationReason` (سبب الإقالة) و `isSettled` (حالة تسليم الراتب الأخير).
- مسارات جديدة للتعامل مع الإقالة بشكل معزول وآمن:
  - `PATCH /api/employees/:id/terminate`: لإنهاء خدمة الموظف مع تسجيل السبب والتاريخ.
  - `PATCH /api/employees/:id/settle`: لتسجيل أن الموظف استلم مستحقاته النهائية (التصفية).
- تعديل جوهري في مسار الرواتب (`processPayrollRun`): أصبح يشمل الموظفين المستقيلين/المقالين الذين لم يستلموا راتبهم الختامي (`isSettled: false`)، مما يضمن نزولهم في مسيرات الرواتب لشهر الإقالة وتصفية حقوقهم.

### 10.5 ملاحظات حول الإجازات (Leave Requests)

- تم إضافة موديل `LeaveRequest` في جدول قاعدة البيانات وحالاته (`PENDING`, `APPROVED`, إلخ).
- يتم تجهيز المسارات والخدمات المتعلقة بالإجازات كـ Workflow متكامل لربطه بمسار الرواتب والحضور مستقبلاً.
